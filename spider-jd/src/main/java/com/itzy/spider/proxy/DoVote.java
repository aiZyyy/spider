...
public class DoVote {
    ...
    private static void multiThread() throws IOException, InterruptedException {
        // 1.创建阻塞队列用来存放代理IP地址
        final ArrayBlockingQueue<String> arrayBlockingQueue = new ArrayBlockingQueue<>(10000);
        // 2.读取文件，该文件中保存着代理ip和端口号。如 103.29.186.189:3128
        List<String> resultList = new ArrayList<>();
        BufferedReader br = new BufferedReader(new FileReader(new File("d:/ips.txt")));
        String s;
        ...
        br.close();
        System.out.println("total ips:" + resultList.size());
        ...
        for (int i = 0; i < size; i++) {
            pool.submit(new Thread(() -> {
                while (true) {
                    try {
                        String ipkv1 = arrayBlockingQueue.take();
                        System.out.println(ipkv1);
                        String[] kv = ipkv1.split(":");
    ...
    private static String getPS(CloseableHttpClient hc) throws IOException, ClientProtocolException {
        HttpGet httpGet = new HttpGet("http://fendou.itcast.cn/article/look/aid/193?qq-pf-to=pcqq.c2c");
    ...
    private static Map<String, String> getHeader() {
        ...
        headers.put("cookie",
                "PHPSESSID=8bj2oolgfq23idj1n3qofmm893; UM_distinctid=15e0d154d8a617-095bb15b8b50fd-414a0229-100200-15e0d154d8b4b3");
